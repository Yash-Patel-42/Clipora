...
function YashTestEditingPage() {
  ...
  useState(() => {
    fetch(`${BACKEND_URL}/processors`)
      ...
      .catch(() => setProcessors(['bg_remover', 'noise_reduction', 'text_apply'])); // fallback
  }, []);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800 py-8 px-2 dark">
      <div className="w-full max-w-2xl bg-gray-900 rounded-2xl shadow-2xl p-8 border border-gray-800">
        <h1 className="text-3xl font-bold text-center text-blue-300 mb-6">Sniply - AI Video Editor</h1>
        
        {/* Video Upload */}
        <div className="mb-6">
          <label htmlFor="video-upload" className="block text-gray-300 font-medium mb-2">1. Upload Video (MP4)</label>
          <input
            id="video-upload"
            type="file"
            accept="video/mp4"
        ...
        <div className="mb-6 p-4 border border-gray-700 rounded-lg">
          <h2 className="text-xl font-semibold text-blue-300 mb-4">2. Text Overlay Controls</h2>
        ...
        {/* Action Buttons */}
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-blue-300 mb-3 text-center">3. Choose an Action</h3>
            ...
            {processors.map((proc) => (
              <button
                ...
                className={`px-5 py-2 rounded-lg font-semibold shadow transition-all duration-200
                  ${selectedProcessor === proc && processing ? 'bg-blue-700 text-white animate-pulse' : ''}
                  ${selectedProcessor === proc && !processing ? 'bg-blue-600 text-white' : 'bg-gray-800 text-blue-300 hover:bg-gray-700'}
                  ${processing || !videoFile ? 'opacity-60 cursor-not-allowed' : ''}`
                }
              >
                {processing && selectedProcessor === proc ? 'Processing...' : proc.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}
              </button>
            ))}
          </div>
        </div>
        ...
        {resultUrl && (
          <div className="mt-6 text-center">
            <h2 className="text-xl font-bold text-green-400 mb-2">4. Your Processed Video</h2>
