...
@export_router.post("/")
async def export_endpoint(file: UploadFile = File(...), resolution: str = Form("1080p")):
    ...
    output_path = os.path.join(output_dir, "exported.mp4")
    with open(input_path, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
    try:
        export_video(input_path, output_path, resolution)
        return FileResponse(output_path, media_type="video/mp4", filename="exported.mp4")
